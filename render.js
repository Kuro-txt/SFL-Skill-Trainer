...
function renderTierGrid(tierNum, elementId) {
  ...
  container.innerHTML = tierSkills.map(skill => {
    ...
    return `
      <div onclick="toggleSkillCard('${skill.id}')" class="${cardClass} p-3 rounded-xl flex flex-col justify-between select-none cursor-pointer transition-all min-h-[110px]">
        <div>
...
function renderSummary() {
  ...
  // Desktop + Mobile Total Badges
  if (document.getElementById('hud-total-points')) document.getElementById('hud-total-points').innerText = `${globalInv.points} pts`;
  if (document.getElementById('hud-total-shards')) document.getElementById('hud-total-shards').innerText = `${globalInv.shards}`;
  if (document.getElementById('hud-mobile-points')) document.getElementById('hud-mobile-points').innerText = `${globalInv.points}p`;
  if (document.getElementById('hud-mobile-shards')) document.getElementById('hud-mobile-shards').innerText = `${globalInv.shards}🔷`;
  ...
  // Active Buffs & Debuffs strictly for current tab
  let buffs = [];
  let debuffs = [];
...
function showToast(message, type = "success") {
  ...
  if (type === "error") {
    ...
    toast.className = "fixed bottom-5 right-5 z-50 flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-600 text-slate-300 shadow-2xl text-xs font-semibold toast-active";
  } else {
...
function renderModalContent() {
  ...
  if (suggestions.length === 0) {
    listContainer.innerHTML = `<div class="p-5 rounded-xl card-sub text-center text-xs text-slate-500 italic">No community presets added for ${activeCat.name} yet.</div>`;
  } else {
